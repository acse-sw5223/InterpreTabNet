import torch
from torch.nn import Linear, BatchNorm1d, ReLU, ELU, PReLU, SiLU
...
def initialize_non_glu(module, input_dim, output_dim):
    ...
    # gain_value = np.sqrt((input_dim + output_dim))
...
class TabNetEncoder(torch.nn.Module):
    def __init__(
        ...
        self.elu = ELU()  # currently best 0.7753
        self.prelu = PReLU()  # activation function
        self.silu = SiLU()  # 0.7727
        self.relu = ReLU()  # 0.7705

        # Learnable weights for each branch
        self.weights = nn.Parameter(torch.tensor([0.6, 0.2, 0.2]))
    ...
    def forward(self, x, prior=None):
        ...
        alpha, beta, cita = F.softmax(self.weights, dim=0)
        for step in range(self.n_steps):
            ...
            # d = ReLU()(out[:, : self.n_d])
            d = alpha * self.elu(out[:, : self.n_d]) + beta * self.prelu(out[:, : self.n_d]) + cita * self.silu(out[:, : self.n_d])
...
#         virtual_batch_size=128,
#         momentum=0.02,
#         mask_type="sparsemax",  # entmax 0.7676  sparsemax 0.7672
#     ):
#         """
...
#         # Learnable weights for each branch
#         self.weights = nn.Parameter(torch.tensor([0.15, 0.8, 0.05]))
#         # 0.8 0.1 0.1   0.7703
#         # 0.8 0.15 0.05 0.7731
#         # 0.8 0.05 0.15 0.7673
#         # 0.9 0.05 0.05 0.7726
#         # 0.9 0.1 0     0.7651
#         # 0.6 0.2 0.2   0.7723
#         # 0.7 0.2 0.1   0.7705
#         # 0.85 0.1 0.05 0.7693

#         # five activative conv + conv6
#         # 0.2 0.7 0.1   0.7694
#         # 0.3 0.5 0.2   0.7719
#         # 0.15 0.8 0.05 0.7757

#         # elu    0.7695 (2)
#         # prelu  0.7733 (1)
#         # silu   0.7642 (3)  
#         # without activation 0.7719
    
#         self.conv1 = nn.Sequential(
#             nn.Conv1d(group_dim, int(group_dim*0.5), kernel_size=3, padding=1),
#             nn.BatchNorm1d(int(group_dim*0.5)),
#             # ELU(),
#         )
#         self.conv2 = nn.Sequential(
#             nn.Conv1d(int(group_dim*0.5), int(group_dim*0.25), kernel_size=3, padding=1),
#             nn.BatchNorm1d(int(group_dim*0.25)),
#             # ELU(),
#         )
#         self.conv3 = nn.Sequential(
#             nn.Conv1d(int(group_dim*0.25), int(group_dim*0.125), kernel_size=3, padding=1),
#             nn.BatchNorm1d(int(group_dim*0.125)),
#             # ELU(),
#         )
#         self.conv4 = nn.Sequential(
#             nn.Conv1d(int(group_dim*0.125), int(group_dim*0.25), kernel_size=3, padding=1),
#             nn.BatchNorm1d(int(group_dim*0.25)),
#             # ELU(),
#         )
#         self.conv5 = nn.Sequential(
#             nn.Conv1d(int(group_dim*0.25), int(group_dim*0.5), kernel_size=3, padding=1),
#             nn.BatchNorm1d(int(group_dim*0.5)),
#             # ELU(),
#         )
#         self.conv6 = nn.Sequential(
#             nn.Conv1d(int(group_dim*0.5), group_dim, kernel_size=3, padding=1),
#             nn.BatchNorm1d(group_dim)
#         )

#         self.project = nn.Conv1d(input_dim, group_dim, kernel_size=1)
#         self.dropout = nn.Dropout(p=0.3)

#     def forward(self, priors, processed_feat):
#         x = self.fc(processed_feat)  # Assuming processed_feat is of shape [batch_size, group_dim]
#         x = self.bn(x)
#         x = torch.mul(x, priors)

#         x = x.unsqueeze(1)  # Shape becomes [batch_size, 1, group_dim]
#         x = x.permute(0, 2, 1)  # Shape becomes [batch_size, group_dim, 1]

#         alpha, beta, cita = F.softmax(self.weights, dim=0)
#         x = alpha * self.elu(self.conv1(x)) + beta * self.prelu(self.conv1(x)) + cita * self.silu(self.conv1(x))
#         x = alpha * self.elu(self.conv2(x)) + beta * self.prelu(self.conv2(x)) + cita * self.silu(self.conv2(x))
#         x = alpha * self.elu(self.conv3(x)) + beta * self.prelu(self.conv3(x)) + cita * self.silu(self.conv3(x))
#         x = alpha * self.elu(self.conv4(x)) + beta * self.prelu(self.conv4(x)) + cita * self.silu(self.conv4(x))
#         x = alpha * self.elu(self.conv5(x)) + beta * self.prelu(self.conv5(x)) + cita * self.silu(self.conv5(x))
#         x = self.conv6(x)
#         # x = self.dropout(x)

#         x = x.permute(0, 2, 1)  # Shape becomes [batch_size, 1, group_dim]
#         x = x.squeeze(1)  # Shape becomes [batch_size, group_dim]
#         x = self.selector(x)
...
#             nn.Linear(group_dim, group_dim),  # First fully connected layer
#             ReLU(),  # Activation function
#             nn.Linear(group_dim, group_dim),
#             ReLU()
#         )
#         # initialize_non_glu 0.7734
...
#         for layer in layers:
#             list_of_layers.extend([nn.Linear(hidden_dim, layer), 
#                                    nn.ReLU(inplace=True), 
#                                    nn.BatchNorm1d(layer),
#                                    nn.Dropout(0.4)])
#             hidden_dim = layer

#         list_of_layers.append(nn.Linear(layers[-1], group_dim))
